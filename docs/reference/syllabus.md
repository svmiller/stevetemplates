...
TRUE, format changes to allow a second teacher. Conditional on
`coteaching` being TRUE *and* there being an `email3` entry, a third
co-teacher is added and entries for the third teacher's office hours and
office are assumed. If `coteaching` is TRUE, `email3` has an entry *and*
there is an `email4` entry, a fourth co-teacher is added.

In the case of the above nested conditionality for co-teachers, be
advised this was a hacky (sic) solution until I got too frustrated with
doing it this way. You may want to look at the `syllabus2` template in
the same package.
