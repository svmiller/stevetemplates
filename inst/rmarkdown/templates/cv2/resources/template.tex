...
%\definecolor{sublue}{rgb}{0,.18431,.37255}
...
$if(address)$\moveleft.5\hoffset\centerline{$if(fontawesome)${\small \faMapMarker* \hspace{.5 mm}}$else$$endif$$address$}$endif$
\moveleft.5\hoffset\centerline{ $if(email)$$if(fontawesome)${\small\faEnvelope[regular]\hspace{1 mm}}$else$\emph{E-mail:}$endif$ \href{mailto:}{\tt $email$} \hspace{1 mm}$endif$ $if(phone)$$if(fontawesome)${\small \faPhone \hspace{1 mm}}$else$\emph{Phone:}$endif$  $phone$  \hspace{1 mm} $endif$ $if(github)$$if(fontawesome)${\small\faGithubSquare \hspace{1 mm}}$else$\emph{Github:}$endif$ \href{http://github.com/$github$}{\tt $github$} \hspace{1 mm} $endif$  $if(twitter)$$if(fontawesome)$\faTwitter \hspace{1 mm}$else$\emph{Twitter:}$endif$ \href{https:/twitter.com/$twitter$}{\tt $twitter$} \hspace{1 mm} $endif$ $if(osf)$$if(fontawesome)$\aiOSF \hspace{1 mm}$else$\emph{osf:}$endif$ \href{https:/osf.io/$osf$}{\tt osf.io/$osf$} \hspace{1 mm} $endif$ $if(orcid)$$if(fontawesome)$\href{https://orcid.org/$orcid$}{\textcolor{orcidlogocol}{\aiOrcid}} \hspace{.5 mm}$else$\emph{ORCID:}$endif$ \href{https://orcid.org/$orcid$}{\tt $orcid$} \hspace{1 mm} $endif$ $if(web)$$if(fontawesome)${\small\faGlobe \hspace{1 mm}}$else$\emph{Web:}$endif$ \href{http://$web$}{\tt $web$}  $endif$ $if(updated)$ | \emph{Updated:} $if(rdateformat)$$rdateformat$$else$\apstylekinda\today$endif$$endif$}
