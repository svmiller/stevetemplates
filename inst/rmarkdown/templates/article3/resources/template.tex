...
$if(author/allbutlast)$$for(author/allbutlast)${$author.name$\textsuperscript{$author.affil-code$$if(remove-emails)$$else$,$author.order$$endif$}$sep$, $endfor$$for(author/last)${, and $author.name$\textsuperscript{$author.affil-code$,$author.order$}}$endfor$$else$${author.name/last}$endif$ \vskip 24.5pt
