...
{$author.name$\textsuperscript{$author.affil-code$$if(remove-emails)$$else$,$author.order$$endif$}$sep$$endfor$
$for(author/last)$
{ and $author.name$\textsuperscript{$author.affil-code$$if(remove-emails)$$else$,$author.order$$endif$}}$endfor$
$else$
${author.name/last}
$endif$ \vskip 24.5pt
$endif$
...
$endif$ % this endif is for removing titles in (anonymous)
...
$for(affils)$
\emph{$affils.address/nowrap$}\par
$endfor$
$else$
\setlength\parskip{-.1em}
		$for(affils)$
...
$if(anonymous)$ % if it's anonymous, provide an abstract at the least.
...
$else$ % if it's not anonymous, but it's still alternate layout
...
\end{minipage}
		...
		$else$
		\noindent $for(author)$
		\textsuperscript{$author.order$} {\tt $author.email/nowrap$}$sep$\par
		$endfor$
		$endif$
		}
		\vskip 8.5pt
...
$endif$ % this endif is for futzing with the abstract in anonymous mode

$else$ % this is for the "default" layout of the abstract area, not in anonymous mode.

$if(anonymous)$ % that said, if it's anonymous mode, provide an abstract at least.
...
$else$ % otherwise, give the people what they want
\vskip -14.5pt
\begin{tabularx}{\linewidth}{@{}X@{}}
\begin{minipage}[t]{0.6\textwidth}
